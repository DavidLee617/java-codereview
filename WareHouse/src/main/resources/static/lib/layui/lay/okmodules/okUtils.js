...
layui.define(["layer"], function (exprots) {
    ...
    var okUtils = {
        ...
        tableBatchCheck1: function (table) {
            var checkStatus1 = table.checkStatus("tableId1");
            var rows = checkStatus1.data.length;
            if (rows > 0) {
                var idsStr = "";
                for (var i = 0; i < checkStatus1.data.length; i++) {
                    idsStr += checkStatus1.data[i].id + ",";
                }
                return idsStr;
            } else {
                layer.msg("未选择有效数据", {offset: "t", anim: 6});
        ...
        tableBatchCheck2: function (table) {
            var checkStatus1 = table.checkStatus("tableId1");
            var rows = checkStatus1.data.length;
            if (rows > 0) {
                var dstate = "";
                for (var i = 0; i < checkStatus1.data.length; i++) {
                    dstate += checkStatus1.data[i].state + ",";
                }
                return dstate;
            } else {
                layer.msg("未选择有效数据", {offset: "t", anim: 6});
